...
const CurvedInput = ({
  ...
  placeholder = 'Enter your email', buttonText = 'Get Started',
  type = 'email', name, ariaLabel,
  theme = 'dark', width = 450, bend = 28, height = 64,
  cornerRadius = 18, borderWidth = 1.5, fontSize = 16,
  backgroundColor, textColor, placeholderColor, borderColor,
  buttonColor, buttonTextColor, iconColor,
  shadowSize = 'md', shadowColor,
  showButton = true, showIcon = true, icon,
  className = '', style
}) => {
  ...
  const handleSurfaceClick = e => {
    ...
    input.focus();
    try { input.setSelectionRange(idx, idx); } catch { /* noop */ }
    setCaretIndex(idx);
  };
  ...
  if (geom && layout) {
    ...
    const ew = chipW * 0.5; const eh = chipH * 0.5;
    ...
    content = (
      <svg ref={svgRef} className="curved-input__svg"
        width={geom.W} height={round2(geom.svgH)}
      ...
      >
        ...
        <g clipPath={`url(#${clipId})`}>
          ...
          {focused && (
            <g key={`${display}-${Math.min(caretIndex, display.length)}`}
        ...
        {showButton && (
          <g className="curved-input__button" role="button" tabIndex={0} aria-label={buttonText}
  ...
  return (
    <form ref={rootRef}
      ...
      <input ref={inputRef} className="curved-input__field"
        type={safeType} inputMode={inputMode} name={name}
        value={val} onChange={handleInputChange}
        onSelect={handleSelect} onKeyUp={handleSelect}
